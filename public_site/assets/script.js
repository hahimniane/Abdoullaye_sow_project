(function () {
  ...
  // Keep customer account access one tap away on every public page. The shared
  // script owns this CTA so legal and future pages cannot accidentally omit it.
  function addCustomerLoginLink(container, mobile) {
    if (!container) return;
    var existing = container.querySelector(
      '[data-customer-login="true"], a[href="https://customer.laawoldigital.com"]'
    );
    if (existing) {
      existing.dataset.customerLogin = "true";
      return;
    }
    ...
    link.textContent = document.documentElement.lang === "en"
      ? "Log in"
      : "Se connecter";
    link.dataset.customerLogin = "true";
    link.className = mobile ? "customer-login-link" : "btn btn-customer-login";
    var businessLink = container.querySelector('a[href="partner.html"]');
    if (businessLink) {
      container.insertBefore(link, businessLink);
    } else {
      container.appendChild(link);
    }
  }

  addCustomerLoginLink(document.querySelector(".nav-cta"), false);
  addCustomerLoginLink(document.getElementById("mobileMenu"), true);
