...
import { lotCustomerFromRow, lotCustomerFromStaffRow, lotCustomerSources, matchLotCustomers } from "./lot-customers.ts";
...
const rows = [
  ...
  { id: "2", name: "Fatou Diallo", phone: "9175550123", email: "fatou@x.co", lastSeenAt: { seconds: 20 } },
  { id: "3", name: "Ibrahima Amadou", phone: "", email: "", lastSeenAt: { seconds: 30 } },
...
test("phone digits and email find the customer; a car never does", () => {
  ...
  assert.deepEqual(matchLotCustomers(rows, "1HGCM"), []);
  // And the car on the legacy row was dropped, not read.
  assert.equal(Object.prototype.hasOwnProperty.call(rows[0], "cars"), false);
});
...
test("staff and remembered customers merge without offering the same person twice", () => {
  ...
  // The saved record wins the tie: it carries a real last-seen.
  ...
  assert.deepEqual(lotCustomerSources([], [{ id: "x", name: "", phone: "", email: "", lastSeenMs: 0 }]), []);
});
