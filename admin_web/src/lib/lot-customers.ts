 ...
 * customer typed onto a ledger activity or a walk-up - a name and a phone
 * number, never a car - and this offers them back as staff type. No Firebase
 * here so `lot-customers.test.ts` runs without a browser.
 */
...
export function lotCustomerFromRow(row: Row): LotCustomer {
  // A customer is a name and a phone number. Any car on a legacy record is
  // ignored - the vehicle belongs to the parking record (filled from its VIN),
  // never to the person.
  return {
    id: s(row.id, 200),
    name: s(row.name, 120),
 ...
 * staff. Saved customers win a tie, because they carry a real last seen; a
