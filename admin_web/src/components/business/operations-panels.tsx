        ...
        <>
          ...
          <div className="panel">
            ...
            {scopedActivities.length === 0 ? (
              ...
              <div className="lot-activity-cards">
                {scopedActivities.map((row) => {
                  ...
                  const meta = text(r.auctionHouse, "") ? `Auction: ${text(r.auctionHouse, "")}` : r.feeOverridden ? "Priced for this job" : "Standard rate";
                  const customer = [text(r.customerName, ""), text(r.customerPhone, "")].filter(Boolean).join(" · ");
                  return (
                    <article className={`lot-activity-card${voided ? " voided" : ""}`} key={String(r.id)}>
                      <div className="lot-activity-card-head">
                        <div className="lot-activity-card-title">
                          <strong style={{ color: tintForType(String(r.activityTypeId)) }}>{label}{voided && <span className="status-pill danger compact" style={{ marginLeft: 8 }}>Voided</span>}</strong>
                          <span className="lot-activity-vehicle">{voided ? <s>{vehicle}</s> : vehicle}{text(r.vinNumber, "") ? ` · ${text(r.vinNumber, "")}` : ""}</span>
                          {customer && <small>{customer}</small>}
                          <small>{formatDate(r.activityDate)} · {meta}{text(r.editedByStaffId, "") ? " · edited" : ""}</small>
                        </div>
                        <div className="lot-activity-card-money">
                          <b>{voided ? <s>{lotFormatCents(Number(r.feeCents) || 0)}</s> : lotFormatCents(Number(r.feeCents) || 0)}</b>
                      ...
                      <div className="lot-activity-card-foot">
                        ...
                        {/* Money marked received off-platform can be set back to
                            not-received if it never actually came in; logged
                            under whoever does it. */}
                        {!voided && lotActivityPaid(r) && String(r.paymentMethod) === "direct" && (<button className="ghost-button" type="button" onClick={() => void revertActivityPayment(String(r.id), vehicle)}><RotateCcw size={13} /> Mark not received</button>)}
                        <span className="lot-activity-card-actions">
